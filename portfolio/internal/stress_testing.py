"""
Stress Testing Module - CORRECTED VERSION
...
CRITICAL FIX: Portfolio log returns cannot be computed as weighted average of 
individual log returns. This version correctly converts to simple returns first,
then aggregates, then converts back to log returns.
"""
...
class StressTester:
    ...
    def replay_period(
        ...
        Uses log returns for calculations and implements a buy-and-hold strategy
        (weights are set at the beginning and drift naturally with price changes).
        
        MATHEMATICAL NOTE:
        Portfolio log returns CANNOT be computed as weighted average of individual
        log returns. The correct formula is:
        1. Convert log returns to simple returns: R_i = exp(r_i) - 1
        2. Portfolio simple return: R_p = Σ(w_i × R_i)
        3. Convert to log return: r_p = ln(1 + R_p)
        ...
        # ============================================================
        # CRITICAL FIX: Correct calculation of portfolio log returns
        # ============================================================
        # We have log returns for individual assets: r_i = ln(P_t / P_{t-1})
        # Portfolio log returns CANNOT be computed as: r_p = Σ(w_i × r_i) [WRONG!]
        # 
        # Correct method:
        # 1. Convert individual log returns to simple returns: R_i = exp(r_i) - 1
        # 2. Calculate portfolio simple return: R_p = Σ(w_i × R_i)
        # 3. Convert portfolio simple return to log return: r_p = ln(1 + R_p)
        
        # Step 1: Convert log returns to simple returns
        simple_returns = np.exp(period_returns[valid_tickers]) - 1
        
        # Step 2: Calculate portfolio simple returns (weighted average)
        portfolio_simple_returns = simple_returns.dot(valid_weights)
        
        # Step 3: Convert portfolio simple returns back to log returns
        portfolio_daily_returns = np.log(1 + portfolio_simple_returns)
